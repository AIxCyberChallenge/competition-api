...
BASE_RUNNER_IMAGE = f"ghcr.io/aixcyberchallenge/base-runner{BASE_IMAGE_TAG}"

BASE_IMAGES = {
    "generic": [
        f"ghcr.io/aixcyberchallenge/base-image{BASE_IMAGE_TAG}",
        f"ghcr.io/aixcyberchallenge/base-clang{BASE_IMAGE_TAG}",
        f"ghcr.io/aixcyberchallenge/base-builder{BASE_IMAGE_TAG}",
        BASE_RUNNER_IMAGE,
        f"ghcr.io/aixcyberchallenge/base-runner-debug{BASE_IMAGE_TAG}",
    ],
    "go": [f"ghcr.io/aixcyberchallenge/base-builder-go{BASE_IMAGE_TAG}"],
    "javascript": [f"ghcr.io/aixcyberchallenge/base-builder-javascript{BASE_IMAGE_TAG}"],
    "jvm": [f"ghcr.io/aixcyberchallenge/base-builder-jvm{BASE_IMAGE_TAG}"],
    "python": [f"ghcr.io/aixcyberchallenge/base-builder-python{BASE_IMAGE_TAG}"],
    "rust": [f"ghcr.io/aixcyberchallenge/base-builder-rust{BASE_IMAGE_TAG}"],
    "ruby": [f"ghcr.io/aixcyberchallenge/base-builder-ruby{BASE_IMAGE_TAG}"],
    "swift": [f"ghcr.io/aixcyberchallenge/base-builder-swift{BASE_IMAGE_TAG}"],
}
...
CORPUS_URL_FORMAT = (
    "gs://{project_name}-corpus.clusterfuzz-external.appspot.com/libFuzzer/" "{fuzz_target}/"
)
CORPUS_BACKUP_URL_FORMAT = (
    "gs://{project_name}-backup.clusterfuzz-external.appspot.com/corpus/" "libFuzzer/{fuzz_target}/"
)
...
CLUSTERFUZZLITE_DOCKER_IMAGE = "ghcr.io/aixcyberchallenge/cifuzz-run-fuzzers"
...
class Project:
    ...
    def __init__(
        ...
        if self.is_external:
            ...
            self.build_integration_path = os.path.join(self.path, build_integration_path)
        else:
...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
    ...
    generate_parser = subparsers.add_parser("generate", help="Generate files for new project.")
    ...
    build_image_parser.add_argument("--pull", action="store_true", help="Pull latest base image.")
    _add_architecture_args(build_image_parser)
    build_image_parser.add_argument(
        "--cache", action="store_true", default=False, help="Use docker cache when building image."
    )
    build_image_parser.add_argument(
    ...
    build_image_parser.add_argument(
        ...
        default="latest",
        help="docker image build tag" "default: latest",
    )
    build_image_parser.add_argument(
        "--err_result",
        default=1,
        help="exit code override for helper.py errors " "(default err_result = 1).",
        type=int,
    )
    build_image_parser.add_argument(
    ...
    build_fuzzers_parser.add_argument("source_path", help="path of local source", nargs="?")
    ...
    check_build_parser.add_argument("project", help="name of the project or path (external)")
    ...
    run_fuzzer_parser.add_argument("project", help="name of the project or path (external)")
    run_fuzzer_parser.add_argument("fuzzer_name", help="name of the fuzzer")
    run_fuzzer_parser.add_argument("fuzzer_args", help="arguments to pass to the fuzzer", nargs="*")
    ...
    fuzzbench_measure_parser.add_argument("project", help="name of the project or path (external)")
    ...
    coverage_parser.add_argument(
        "--fuzz-target",
        help="specify name of a fuzz " "target to be run for generating coverage " "report",
    )
    coverage_parser.add_argument(
        "--corpus-dir",
        help="specify location of corpus" " to be used (requires --fuzz-target argument)",
    )
    coverage_parser.add_argument(
        "--public", action="store_true", help="if set, will download public " "corpus using wget"
    )
    coverage_parser.add_argument("project", help="name of the project or path (external)")
    coverage_parser.add_argument(
        "extra_args", help="additional arguments to " "pass to llvm-cov utility.", nargs="*"
    )
    ...
    introspector_parser.add_argument("source_path", help="path of local source", nargs="?")
    introspector_parser.add_argument(
        "--public-corpora",
    ...
    download_corpora_parser.add_argument("project", help="name of the project or path (external)")

    reproduce_parser = subparsers.add_parser("reproduce", help="Reproduce a crash.")
    reproduce_parser.add_argument("--valgrind", action="store_true", help="run with valgrind")
    ...
    reproduce_parser.add_argument(
        "--err_result",
        help="exit code override for missing harness / fuzz targets " "(default err_result = 1).",
        type=int,
    )
    reproduce_parser.add_argument(
        "--timeout",
        help="timeout for reproduce subprocess " "(default: None).",
        default=None,
        type=int,
    )
    reproduce_parser.add_argument("project", help="name of the project or path (external)")
    ...
    reproduce_parser.add_argument("fuzzer_args", help="arguments to pass to the fuzzer", nargs="*")
...
def _check_fuzzer_exists(project, fuzzer_name, architecture="x86_64"):
    ...
    try:
        ...
        logger.error("%s does not seem to exist. Please run build_fuzzers first.", fuzzer_name)
        return False
...
def _add_architecture_args(parser, choices=None):
    ...
    parser.add_argument("--architecture", default=constants.DEFAULT_ARCHITECTURE, choices=choices)
...
def build_image_impl(
    ...
    if is_base_image(image_name):
        image_project = "aixcyberchallenge"
...
def docker_run(
    ...
    if privileged:
        ...
        command = ["docker", "run", "--shm-size=2g", "--platform", platform]
    ...
    try:
        subprocess.check_call(command, stdout=stdout, stderr=subprocess.STDOUT, timeout=timeout)
...
def build_fuzzers_impl(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    ...
    if build_project_image and not build_image_impl(
        project, architecture=architecture, docker_image_tag=docker_image_tag
    ):
        return err_result if propagate_exit_codes else False
    ...
    command += ["-v", f"{project_out}:/out", "-v", f"{project.work}:/work", f"{docker_image}"]
    ...
    result = docker_run(
        command, architecture=architecture, propagate_exit_codes=propagate_exit_codes
    )
...
def run_clusterfuzzlite(args):
    ...
    try:
        with tempfile.TemporaryDirectory() as workspace:
            ...
            build_command = [
                "--tag",
                "ghcr.io/aixcyberchallenge/cifuzz-run-fuzzers",
                "--file",
...
def fuzzbench_build_fuzzers(args):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        subprocess.run(
            ["git", "clone", "https://github.com/google/fuzzbench", "--depth", "1", fuzzbench_path],
            check=True,
        )
        env = [
        ...
        subprocess.run(
            [
                "docker",
                "tag",
                "ghcr.io/aixcyberchallenge/base-builder-fuzzbench",
                f"ghcr.io/aixcyberchallenge/base-builder{BASE_IMAGE_TAG}",
            ],
            check=True,
        )
...
def check_build(args):
    ...
    result = docker_run(
        run_args, architecture=args.architecture, propagate_exit_codes=args.propagate_exit_codes
    )
...
def _get_latest_corpus(project, fuzz_target, base_corpus_dir):
    ...
    if output:
        ...
        corpus_url = CORPUS_URL_FORMAT.format(project_name=project.name, fuzz_target=fuzz_target)
    ...
    try:
        with open(os.devnull, "w") as stdout:
            subprocess.check_call(
                ["unzip", "-q", "-o", target_zip, "-d", target_fuzzer_dir], stdout=stdout
            )
    except OSError:
...
def download_corpora(args):
    ...
    logger.info("Downloading corpora for %s project to %s.", args.project.name, corpus_dir)
...
def coverage(args):  # pylint: disable=too-many-branches
    ...
    if args.corpus_dir and not args.fuzz_target:
        logger.error(
            "--corpus-dir requires specifying a particular fuzz target using " "--fuzz-target"
        )
        return False
    ...
    if args.project.language not in constants.LANGUAGES_WITH_COVERAGE_SUPPORT:
        logger.error(
            "Project is written in %s, coverage for it is not supported yet.", args.project.language
        )
        return False

    if not args.no_corpus_download and not args.corpus_dir and not args.project.is_external:
        if not download_corpora(args):
            return False

    extra_cov_args = f'{args.project.coverage_extra_args.strip()} {" ".join(args.extra_args)}'
    env = [
...
def _introspector_prepare_corpus(args):
    ...
    if args.private_corpora or args.public_corpora:
        ...
        for fuzzer_name in fuzzer_targets:
            ...
            parsed_args.fuzzer_args = [f"-max_total_time={args.seconds}", "-detect_leaks=0"]
...
def introspector(args):
    ...
    # Collect coverage.
    coverage_command = ["coverage", "--no-corpus-download", "--port", "", args.project.name]
...
def reproduce_impl(  # pylint: disable=too-many-arguments
    ...
    run_args = (
        ...
        + [
            ...
            "-t",
            "ghcr.io/aixcyberchallenge/%s%s" % (image_name, BASE_IMAGE_TAG),
            "reproduce",
            fuzzer_name,
            "-runs=100",
        ]
    ...
    return run_function(
        run_args,
        architecture=architecture,
        propagate_exit_codes=propagate_exit_codes,
...
def _validate_project_name(project_name):
    ...
    if len(project_name) > MAX_PROJECT_NAME_LENGTH:
        logger.error(
            "Project name needs to be less than or equal to %d characters.", MAX_PROJECT_NAME_LENGTH
        )
        return False
...
def pull_images(language=None):
    ...
    for base_image_lang, base_images in BASE_IMAGES.items():
        if language is None or base_image_lang == "generic" or base_image_lang == language:
